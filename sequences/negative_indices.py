# ============================================================
#                LESSON — NEGATIVE INDICES
# ============================================================
# Author: Alexandru Petrenco (with AI assistance from OpenAI GPT-5)
#
# Description:
#   This lesson explains how negative indexing works in Python:
#       - what negative indices are
#       - why they exist
#       - how to use them with lists, strings, tuples
#       - negative slicing
#       - out-of-range negatives
#       - where negative indexing does NOT work
#
# Each section prints a separator for clean, educational output.
# ============================================================


# ============================================================
# 1. BASIC IDEA OF NEGATIVE INDICES
# ============================================================

print("\n--- SECTION 1: Basic Idea of Negative Indices ---")

nums = [10, 20, 30, 40, 50]

print(nums[-1])   # last element → 50
print(nums[-2])   # second from end → 40
print(nums[-3])   # third from end → 30


# ============================================================
# 2. WHY NEGATIVE INDICES EXIST
# ============================================================

print("\n--- SECTION 2: Why Negative Indices Exist ---")

# Traditional way:
print(nums[len(nums) - 1])  # 50

# Cleaner way:
print(nums[-1])             # also 50


# ============================================================
# 3. NEGATIVE INDICES IN DIFFERENT SEQUENCES
# ============================================================

print("\n--- SECTION 3: Negative Indices in Multiple Collections ---")

my_list = [1, 2, 3]
my_tuple = (4, 5, 6)
my_string = "Python"

print(my_list[-1])   # 3
print(my_tuple[-2])  # 5
print(my_string[-3]) # 'h'


# ============================================================
# 4. NEGATIVE INDICES WITH SLICING
# ============================================================

print("\n--- SECTION 4: Negative Indices in Slicing ---")

letters = ["a", "b", "c", "d", "e"]

print(letters[-3:])      # ['c', 'd', 'e']
print(letters[:-2])      # ['a', 'b', 'c']
print(letters[-4:-1])    # ['b', 'c', 'd']

# Mixed example:
print(letters[1:-1])     # everything except first and last


# ============================================================
# 5. USING NEGATIVE STEP (REVERSING)
# ============================================================

print("\n--- SECTION 5: Negative Step (Reversing Sequences) ---")

nums = [1, 2, 3, 4, 5]
print(nums[::-1])   # reversed → [5, 4, 3, 2, 1]

# Partial reverse:
print(nums[4:1:-1])  # [5, 4, 3]


# ============================================================
# 6. OUT-OF-RANGE NEGATIVE INDICES
# ============================================================

print("\n--- SECTION 6: Out-of-Range Negative Indices ---")

test = [1, 2, 3]

try:
    print(test[-4])  # error → too negative
except Exception as e:
    print(f"Error: {type(e).__name__}")  


# ============================================================
# 7. COLLECTIONS THAT DO *NOT* SUPPORT NEGATIVE INDICES
# ============================================================

print("\n--- SECTION 7: Collections Without Negative Indices ---")

# Sets and dicts have no meaningful order
my_set = {1, 2, 3}
my_dict = {"a": 1, "b": 2}

for obj in (my_set, my_dict):
    try:
        print(obj[-1])
    except Exception as e:
        print(f"{type(obj).__name__} → Error: {type(e).__name__}")


# ============================================================
# 8. CUSTOM CLASS WITH NEGATIVE INDEX SUPPORT
# ============================================================

print("\n--- SECTION 8: Custom Class Supporting Negative Indices ---")

class MySeq:
    def __init__(self, data):
        self.data = data

    def __getitem__(self, idx):
        if idx < 0:
            idx = len(self.data) + idx
        return self.data[idx]

custom = MySeq(["x", "y", "z"])
print(custom[-1])   # 'z'
print(custom[-2])   # 'y'
